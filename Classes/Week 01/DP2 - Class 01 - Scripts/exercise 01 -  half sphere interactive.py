...
xCount = rs.GetInteger("How many in X?")
yCount = rs.GetInteger("How many in Y?")

thetaValues = np.arange(0, 2 * math.pi, 2 * math.pi /xCount) # 0 to 2PI in xCount steps
alphaValues = np.arange(0, math.pi / 2.0, math.pi / 2.0 / yCount) # 0 to PI / 2 in yCount steps
...
for theta in thetaValues:
    ...
    for alpha in alphaValues:
        ...
        if (i > 0): rs.AddLine(pointIds[i-1][j], pointIds[i][j]) # if this isn't the first column
        if (j > 0): rs.AddLine(pointIds[i][j-1], pointIds[i][j]) # if this isn't the first row
        if (i >= xCount - 1):  rs.AddLine(pointIds[0][j], pointIds[i][j]) # if this IS last column
